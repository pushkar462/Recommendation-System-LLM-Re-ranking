...
def build_feature_vector(
    ...
    ctx     = context_score(user_context, item) if user_context else 0.5
...
class PairwiseRerankerDataset(Dataset):
    def __init__(
        ...
        self.pairs = self.pairs[:5000]   # ← ADD THIS LINE HERE
    ...
    def __getitem__(self, idx):
        ...
        pos_feat = build_feature_vector(pos_item, history, self.genre_to_id, self.n_genres)
        neg_feat = build_feature_vector(neg_item, history, self.genre_to_id, self.n_genres)
...
class NeuralReranker:
    ...
    def train(
        ...
        ds     = PairwiseRerankerDataset(ratings, items_by_id, genre_to_id, n_genres, seed=seed)
    ...
    def _mmr(
        ...
        while len(selected_idx) < top_k and remaining_idx:
            if not selected_idx:
                ...
                selected_genres = {candidates[j].get("genre", "") for j in selected_idx}

                def mmr_score(i):
                    rel = float(scores[i])
                    sim = 1.0 if candidates[i].get("genre", "") in selected_genres else 0.0
                    return lambda_mmr * rel - (1.0 - lambda_mmr) * sim
    ...
    def rerank(
        ...
        scores = self._score_candidates(candidates, user_history, user_context)
        ...
        # MMR diversity pass
        ranked = self._mmr(candidates, scores_ui, top_k=top_k, lambda_mmr=lambda_mmr)
